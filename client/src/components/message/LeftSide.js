...
const LeftSide = () => {
    ...
    const unRead = (user) => {
        return message.users.every((item) => item.unread === 0 || item._id === user._id);
    };
    ...
    useEffect(() => {
        const newMessage = message.users.find((user) => user._id === id);
        if (newMessage) {
            dispatch({ type: MESS_TYPES.CHECK_READ_MESSAGE, payload: { ...newMessage, text: "" } });
        }
                                    ...
                                    {user.online ? (
                                        ...
                                        auth.user.following.find((item) => item._id === user._id) && (
                                            <>
                                                ...
                                                {message.data.length > 0 && user.unread > 0 && (
